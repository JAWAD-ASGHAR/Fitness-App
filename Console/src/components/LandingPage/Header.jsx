    ...
    <div className="w-full bg-[#EDEEF2] dark:bg-slate-800 h-20 flex items-center justify-around">
          ...
          <ul className="flex gap-8">
            {links.map((link) => (
              <a key={link.id} href={link.route}>
                <li className="dark:text-white text-slate-800 font-medium cursor-pointer hover:text-[#C1E503]">
                  {link.name}
                </li>
              </a>
            ))}
      ...
      <div className="flex gap-10 items-center">
        <Button
          ...
          className="dark:text-white font-medium text-slate-800 p-4"
        >
          Login
        </Button>
        <Button
          color="yellow"
          onClick={() => loginWithRedirect()}
          className="font-medium rounded-lg px-4"
        >
          Start Free
        </Button>
      </div>
